<template>
  <div class="admin">
    ...
    <div class="admin-content">
      <div class="remark">
        <h3>修改第几张就上传第几张即可， 共四张, 点击下图中图片图标上传</h3>
      </div>
      <div class="essay">
        <div class="form">
          <h4>上传第一张图</h4>
  ...
  data () {
    return {
      ...
      uploadimage: {
        ...
        url: getApi().baseURL+"/upcomintrucpic?index=1"
      },
       uploadimage2: {
        // url: 'http://localhost:8080/api/uploadimage?index=2'
        url: getApi().baseURL+"/upcomintrucpic?index=2"
      },
       uploadimage3: {
        // url: 'http://localhost:8080/api/uploadimage?index=3'
         url: getApi().baseURL+"/upcomintrucpic?index=3"
      },
       uploadimage4: {
        // url: 'http://localhost:8080/api/uploadimage?index=4'
         url: getApi().baseURL+"/upcomintrucpic?index=4"
      },
      uploadimage5: {
        // url: 'http://localhost:8080/api/uploadimage?index=4'
         url: getApi().baseURL+"/upcomintrucpic?index=5"
      },
      options: {
        linkify: true,
        highlight(str, lang) {
